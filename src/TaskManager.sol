...
contract TaskManager {
    ...
    function createNewTask(string memory _title, string memory _description ) public {
        ...
        uint task_id = tasks[msg.sender].length+1;
    ...
    function completeTask(uint _id) public onlyTaskOwner(_id) {
        tasks[msg.sender][_id-1].completed = !tasks[msg.sender][_id-1].completed;
        emit TaskCompleted(msg.sender, _id);
    }

    function editTask(uint _id , string memory newTitle,string memory newDescription) public onlyTaskOwner(_id) {
    ...
    function deleteTask(uint _id) public onlyTaskOwner(_id) {
        for (uint i=_id-1; i<tasks[msg.sender].length-1 ; i++) 
        {
    ...
    function getTask(uint _id)public view onlyTaskOwner(_id) returns (string memory, string memory, bool) {
        return (tasks[msg.sender][_id-1].title,tasks[msg.sender][_id-1].description ,tasks[msg.sender][_id-1].completed);
    }
